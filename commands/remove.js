...
module.exports = {
  ...
  run: async function (client, message, args) {
   const queue = message.client.queue.get(message.guild.id);
    if (!queue) return sendError(`There Is Nothing Playing In This Server!\n Use \`${client.config.prefix}play [songname]\` To Play A Song.`,message.channel).catch(console.error);
